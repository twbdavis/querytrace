...
import { useAppStore, useHighlight } from '@/store/useAppStore';
...
const ResultRow = memo(function ResultRow({ row, index, sources, highlighted, animate, virtual }: {
  ...
  return (
    <tr
      ...
      className={`result-data-row transition-colors duration-150 ${animate ? 'result-row-enter' : ''} ${sources ? 'cursor-pointer' : ''}`}
      style={{
    ...
    >
      {row.map((value, ci) => (
        <td key={ci} className={`whitespace-nowrap border-b border-r border-line/60 border-r-[rgba(255,255,255,0.04)] px-1.5 py-0 leading-4 last:border-r-0 ${
          ci === 0 ? `border-l-2 ${highlighted ? 'border-l-accent-result' : 'border-l-transparent'}` : ''
  ...
  return (
    <div ref={viewport} data-result-scroll tabIndex={0} aria-label="Scrollable query results" className="min-h-0 flex-1 overflow-auto px-3 pb-3 max-sm:px-2.5" onMouseLeave={() => setHoveredResultRow(null)}>
      <table
        ...
        style={virtual ? { tableLayout: 'fixed', minWidth: `calc(${widths.reduce((sum, width) => sum + width, 0)}ch + ${columns.length * 16}px)` } : undefined}
      >
        {virtual && <colgroup>{widths.map((width, i) => <col key={i} style={{ width: `calc(${width}ch + 16px)` }} />)}</colgroup>}
        <thead className="sticky top-0 z-10">
          <tr className="result-data-row" aria-rowindex={virtual ? 1 : undefined}>
            {columns.map((column, i) => <th key={i} className="whitespace-nowrap border-b border-line-strong bg-node-header px-1.5 py-1 text-left font-bold text-ink-dim">{column}</th>)}
          </tr>
        </thead>
        <tbody>
          ...
          {rows.slice(start, end).map((row, offset) => {
            ...
            return <ResultRow key={index} row={row} index={index} sources={sources?.[index]} highlighted={highlight?.resultRows.has(index) ?? false} animate={entering && index < 12} virtual={virtual} />;
          })}
